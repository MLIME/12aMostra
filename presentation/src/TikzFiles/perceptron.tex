...
% edges
\path[tedge] (x1) edge node[above=1.8pt] {$\vect{\theta}_{1}$} (vk);
\path[tedge] (x2) edge node[above=0.2pt] {$\vect{\theta}_{2}$} (vk);
\path[tedge] (x3) edge node[above=3.8pt] {$\vect{\theta}_{3}$} (vk);
